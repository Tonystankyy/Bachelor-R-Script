...
Iraq_tokens %>% 
  ...
  labs(x = "Rankings (log)", y = "Frequency (log)", title = "Pre-preprocessing") # figure 4.1.1
...
Ukraine_tokens %>% 
  ...
  ggrepel::geom_label_repel(aes(label = word)) +
  ggdark::dark_theme_classic() +
  labs(x = "Rankings (log)", y = "Frequency (log)", title = "Pre-preprocessing") # equivalent to figure 4.1.1 but in the Ukraine case
...
ukraine_stem %>% tidy() %>%
  ...
  geom_smooth(method = "lm", se = FALSE) +
  ggdark::dark_theme_classic() +
  labs(x = "Rankings (log)", y = "Frequency (log)", title = "Post-preprocessing") # equivalent to figure 4.1.2
...
iraq_stem %>% tidy() %>%
  ...
  geom_smooth(method = "lm", se = FALSE) +
  ggdark::dark_theme_classic() +
  labs(x = "Rankings (log)", y = "Frequency (log)", title = "Post-preprocessing") # figure 4.1.2
...
## distribution of topics according to size and proportion (figure 5.1.1)
...
# distribution of topics according to size and proportion (figure 5.1.2)
...
ggplot(response_freq_combined, aes(x = reorder(word, -ukraine_freq), y = ukraine_freq, fill = "ukraine_freq")) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) # figure 5.1.3
...
sentiment_scores <- data.frame(Corpora = c("Ukraine", "Iraq"), 
                               Score = c(ukraine_score, iraq_score), 
...
ggplot(sentiment_scores, aes(x = Corpora, y = Score, fill = Sentiment)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Sentiment Scores for Ukraine and Iraq Corpora", x = NULL, y = "Sentiment Score") +
